      ...
      <div className="modal-box">
        ...
        <div className="modal-actions">
          ...
          <button className="modal-btn" style={{ background: confirmColor || "var(--blue)", color: "#fff", minWidth: 90 }}
            ...
            {saving ? <span className="btn-spinner" style={{ borderTopColor: "#fff", borderColor: "rgba(255,255,255,0.25)" }} /> : confirmLabel}
          </button>
        </div>
      </div>
    </div>
...
function SettingsPage({ userRole, userName, user, onUserUpdate, token, addLog }) {
  ...
  return (
    <>
      ...
      {confirmSave && <ConfirmModal icon="👤" iconColor="var(--blue)" title={`Save Changes for ${confirmSave.name}?`} message={`Role → ${getDraft(confirmSave).role} · Department → ${getDraft(confirmSave).department}`} confirmLabel="Yes, Save" onConfirm={doSave} onCancel={() => setConfirmSave(null)} saving={confirmSaving} />}
